...
overlay at every viewport size.

- **Type:** `'xs' | 'sm' | 'md' | 'lg' | 'xl' | 'xxl' | 'all'`
- **Default:** `–`
...
A custom breakpoint value that overrides `breakPoint`. Any valid CSS length.

- **Type:** `string`
- **Default:** `–`

```jsx
<Sidebar customBreakPoint="900px">…</Sidebar>
```
...
| `breakPoint` | `'xs' \| 'sm' \| 'md' \| 'lg' \| 'xl' \| 'xxl' \| 'all'` | – | Built-in responsive breakpoint |
| `customBreakPoint` | `string` | – | Custom max-width breakpoint (overrides `breakPoint`) |
